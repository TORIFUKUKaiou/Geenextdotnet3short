...
public class TorifukuBrowser implements TorifukuBrowserInterface {
	...
	public TorifukuBrowser(Activity activity, WebView webView) {
		super();
		mActivity = activity;
		mWebView = webView;
	...
	public void start() {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
			...
			/** ProgressDialog */
			boolean finishing = TorifukuBrowser.this.mActivity.isFinishing();
			if (finishing) {
				return;
			}
	...
	/* (non-Javadoc)
	 * @see jp.torifuku.memobrowser.TorifukuBrowserInterface#getTitle()
	 */
	@Override
		...
		mWebView.setWebChromeClient(new WebChromeClient() {
			...
			public void onProgressChanged(WebView view, int progress) {
				...
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
					...
					TorifukuBrowser.this.mActivity.setProgressBarIndeterminateVisibility(visible);
				} else {
					if (progress >= 100) {
						...
						if (progress > 0) {
							...
							if (finishing) {
								if (BuildConfig.DEBUG) {
									Toast.makeText(TorifukuBrowser.this.mActivity, "finishing is true.", Toast.LENGTH_SHORT).show();
									android.util.Log.i("TEST", "finishing is true.");
								}
								return;
							}
							TorifukuBrowser.this.mProgressDialog = new ProgressDialog(TorifukuBrowser.this.mActivity);
							TorifukuBrowser.this.mProgressDialog.setCancelable(true);
							TorifukuBrowser.this.mProgressDialog.setCanceledOnTouchOutside(true);
	...
	private class CaptureTask extends AsyncTask<Void, Void, CaptureTaskResult> {
		...
		protected void onPostExecute(CaptureTaskResult result) {
			...
			if (result == CaptureTaskResult.Success) {
				builder.setNeutralButton(android.R.string.dialog_alert_title, new OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						TorifukuLog.methodIn();
						if (CaptureTask.this.mCaptureListener != null) {
